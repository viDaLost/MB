...
export const BUNKER_TONES: Record<BunkerTone, { name: string; note: string; description: string }> =
  {
    ...
    absurd: {
      name: 'Абсурд',
      note: 'чёрный юмор',
      description: 'Странные детали и неожиданные секреты без логических противоречий.',
    },
  };
...
export const SECRETS: Record<BunkerTone, string[]> = {
  realistic: [
    'скрыл серьёзную ошибку на прошлой работе',
    'умеет подделывать пропуска и документы',
    'имеет доступ к закрытому складу поблизости',
    'раньше участвовал в экспедициях по этой местности',
    'знает координаты автономной станции',
    'сохраняет запас жизненно важного лекарства',
    'был членом конфликтной группы выживших',
    'не сообщил о близком родственнике снаружи',
  ],
  cinematic: [
    'знает аварийный код одного из модулей',
    'получил зашифрованный сигнал перед катастрофой',
    'работал на объекте под другим именем',
    'может вывести группу через технический тоннель',
    'видел исправный транспорт в соседнем секторе',
    'носит ключ от оружейного сейфа',
    'знает настоящую причину катастрофы',
    'поддерживает тайную связь с другой группой',
  ],
  absurd: [
    'официально сертифицирован как переговорщик с голубями',
    'убеждён, что кофемашина является членом команды',
    'тайно коллекционирует аварийные сирены',
    'может безошибочно предсказывать вкус консервов',
    'выиграл чемпионат по молчанию',
    'знает все рекламные джинглы наизусть',
    'носит дипломат с единственным идеальным носком',
    'считает себя министром внутреннего огорода',
  ],
};
...
export const ACTION_CARDS: ActionCardDefinition[] = [
  {
    id: 'shield',
    name: 'Право вето',
    description: 'Защищает вас от ближайшего исключения. После голосования карта сгорает.',
    type: 'shield',
  },
  {
    id: 'intel',
    name: 'Честное признание',
    description:
      'Немедленно публично раскрывает ваш секрет и даёт группе дополнительную информацию.',
    type: 'intel',
  },
  {
    id: 'water',
